...
public class StudentController : ControllerBase
{
   ...
   public StudentController(ApplicationDbcontext context){
    ...
    public async Task<ActionResult<List<Student>>> GetStudents(){
        var students = await _context.SonaStudentwasmTask2.Include(s=>s.Department).ToListAsync();
        return Ok(students);
    }
    ...
    public async Task<ActionResult<Student>> GetStudentById(int id){
        var student = await _context.SonaStudentwasmTask2.Include(s=>s.Department).FirstOrDefaultAsync(s=>s.Id==id);
        if(student==null){
    ...
    public async Task<ActionResult<List<Student>>> CreateStudent([FromBody] Student student){
        ...
        try{
            _context.SonaStudentwasmTask2.Add(student);
    ...
    public async Task<ActionResult<List<Student>>> UpdateStudent(Student student, int id){
        ...
        var dbstudent = await  _context.SonaStudentwasmTask2.FindAsync(id);
        if(dbstudent==null)
        ...
        //dbstudent.DepartId=student.DepartId;
    ...
    public async Task<ActionResult<List<Student>>> DeleteStudent(int id){     
        ...
        if(dbstudent==null)
            return NotFound("Sorry, no student ");
        _context.SonaStudentwasmTask2.Remove(dbstudent);
        await _context.SaveChangesAsync();
        return Ok(await GetDbStudents());
    }
    ...
    private async Task<List<Student>> GetDbStudents(){
        return await _context.SonaStudentwasmTask2.Include(s=>s.Department).ToListAsync();
    }
